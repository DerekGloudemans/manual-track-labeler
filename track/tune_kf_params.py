...
# data
from util_MOT.MOT_tracking_dataset import Tracking_Dataset
                ...
                for idx,item in enumerate(relevant_ims):
                    with Image.open(item) as im:
                           ...
                           if frame.shape[1] < 1080:
                               new_frame = torch.zeros([3,1080,frame.shape[2]])
                               new_frame[:,:frame.shape[1],:] = frame
                               frame = new_frame
                           if frame.shape[2] < 1920:
                               new_frame = torch.zeros([3,1080,1920])
    ...
    for iteration in range(n_iterations):
        ...
        with torch.no_grad():   
            ...
            for i, obj in enumerate(gts):
                ...
                if max_iou < -1: #0.25: #object was missed so don't count it
                    pass
                else:
                    keepers.append(i)
                    ious.append(max_iou)
...
if __name__ == "__main__":
    ...
    try:
        ...
        data_dir = "/home/worklab/Data/cv/MOT20/comb_17_20_train"
        dataset = Tracking_Dataset(data_dir,n = (n_pre + n_post+1))
    ...
    cp = os.path.join(os.getcwd(),"./config/localizer.pt")
    ...
    cp = os.path.join(os.getcwd(),"cpu_MOT_localizer_resnet50_e49.pt")
    retinanet = resnet50(14)
    retinanet.load_state_dict(torch.load(cp))
    retinanet = retinanet.to(device)
    retinanet.eval()
    ...
    # Create the detector
    cp = os.path.join(os.getcwd(),"cpu_MOT_detector_resnet50_e28.pt")
    retinanet = resnet50(14)
    retinanet.load_state_dict(torch.load(cp))
    retinanet = retinanet.to(device)
    retinanet.eval()
    ...
    try:
        ...
        det_dataset = Detection_Dataset(data_dir)
        params = {'batch_size' : 1,
                      'shuffle'    : True,
                      'num_workers': 0,
                      'drop_last'  : True,
