...
def annotate_3d_box(box_queue,results_queue,CONTINUE,vp):
    ...
    while CONTINUE_COPY:
        ...
        # fit box
        box_3d = fit_3D_boxes(diff,box,vp[0],vp[1],vp[2],granularity = 1e-04,e_init = 3e-01,show = False, verbose = False,obj_travel = direction)
        
        result = [box_3d,obj_idx,frame_idx,diff,vp,frame]
        results_queue.put(result)
        
        
def process_boxes(sequence,label_file,downsample = 1,SHOW = True, timeout = 20,threshold = 30,frequency = 1):
        ...
        while all_results_count < len(box_labels) - removed_boxes: 
            ...
            if count <  len(box_labels):
                ...
                if frame_idx != labeled_frame_idx:
                    ...
                    # slowly replace average frame
                    replace = 0.9995
                    avg_frame = cv2.addWeighted( avg_frame, replace,frame,1.0-replace,0.0)
                ...
                if obj_idx not in last_frame_queued_per_object.keys():
                    last_frame_queued_per_object[obj_idx] = frame_idx
                else:
                    if frame_idx < last_frame_queued_per_object[obj_idx] + frequency:
                        removed_boxes += 1
                        count += 1
                        continue
                    else:
        ...
        for sequence in sequences:
            ...
            try:
                ...
                try:
                    ...
                    continue
                process_boxes(vid_sequence,labels,downsample = 1,SHOW = SHOW,threshold = threshold,timeout = 20,frequency = 4)    
